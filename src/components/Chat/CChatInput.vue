<template>
  <div
    class="w-full max-w-5xl mx-auto border-t border-[#30363D] rounded-t-xl bg-[#0D1117] px-4 md:px-6 lg:px-8 py-4 md:py-5"
  >
    <div class="container-grid">
      <div
        class="header-template max-h-32 flex flex-wrap gap-4 overflow-y-scroll"
      >
        <FileCard
          v-for="(item, index) in files"
          :key="index"
          :file="item"
          @click:close="() => handleClickExcludeFile(item)"
        />
      </div>

      <div
        ref="refIconActions"
        class="after-template flex justify-center items-center w-full h-full"
      >
        <button class="text-4xl" @click="() => handleClickBtnMenu()">
          &#x002B;
        </button>
      </div>

      <div class="input-template relative">
        <textarea
          ref="textareaRef"
          v-model="message"
          :placeholder="placeholder"
          :disabled="disabled"
          @keydown="handleKeyDown"
          @input="handleInput"
          class="w-full px-4 md:px-5 py-3 md:py-4 pr-14 rounded-2xl border-2 border-[#30363D] focus:border-primary focus:outline-none transition-all duration-200 resize-none max-h-32 overflow-y-auto bg-[#161B22] text-white placeholder-gray-500 text-sm md:text-base"
          rows="1"
        ></textarea>

        <!-- Contador de caracteres (opcional) -->
        <span
          v-if="showCharCount && maxLength"
          :class="[
            'absolute bottom-4 right-4 text-xs',
            message.length > maxLength ? 'text-red-500' : 'text-gray-500',
          ]"
        >
          {{ message.length }}/{{ maxLength }}
        </span>
      </div>

      <button
        @click="handleSend"
        :disabled="disabled || !canSend"
        class="action-template"
        :class="[
          'flex-shrink-0 w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center transition-all duration-200 shadow-lg',
          canSend
            ? 'bg-gradient-to-br from-primary to-secondary text-white hover:shadow-xl transform hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none'
            : 'bg-background-alt text-text-muted cursor-not-allowed',
        ]"
      >
        <svg
          v-if="!isLoading"
          class="w-5 h-5 md:w-6 md:h-6"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
          <path
            stroke-linecap="round"
            stroke-linejoin="round"
            stroke-width="2"
            d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"
          ></path>
        </svg>
        <div
          v-else
          class="w-5 h-5 md:w-6 md:h-6 border-2 border-white border-t-transparent rounded-full animate-spin"
        ></div>
      </button>
    </div>
  </div>

  <CMenu v-model="isOpenMenuAction" :reference="$refs.refIconActions" width="13rem">
    <template #header>
      <h2 class="text-white font-semibold text-center">Menu</h2>
    </template>
    <ol>
      <li class="p-2 cursor-pointer hover:bg-[#30363D]">
        <button
          class="flex justify-start items-center gap-2 w-full"
          @click="openFilePicker"
        >
          <svg
            fill="#ffffff"
            width="20px"
            height="20px"
            viewBox="0 0 32 32"
            version="1.1"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M23.113 7.748h-0.002c-0 0-0.001 0-0.001 0-0.413 0-0.748 0.335-0.749 0.748v0l-0.029 14.197c0.004 0.084 0.006 0.183 0.006 0.282 0 1.812-0.755 3.448-1.967 4.611l-0.002 0.002c-1.134 1.011-2.639 1.629-4.287 1.629-0.030 0-0.059-0-0.088-0.001l0.004 0h-0.015c-0.028 0-0.061 0.001-0.094 0.001-1.64 0-3.136-0.611-4.275-1.618l0.007 0.006c-1.225-1.165-1.988-2.808-1.988-4.629 0-0.090 0.002-0.18 0.006-0.269l-0 0.013v-15.516c-0.003-0.056-0.004-0.122-0.004-0.188 0-2.341 1.898-4.239 4.239-4.239 0.066 0 0.131 0.002 0.196 0.004l-0.009-0c0.056-0.003 0.122-0.004 0.188-0.004 2.341 0 4.238 1.897 4.238 4.238 0 0.065-0.002 0.131-0.004 0.195l0-0.009-0.057 14.276c0.002 0.045 0.003 0.097 0.003 0.15 0 0.85-0.311 1.628-0.826 2.225l0.004-0.004c-0.411 0.439-0.994 0.712-1.641 0.713h-0.020c-0.654-0.006-1.241-0.29-1.648-0.74l-0.002-0.002c-0.489-0.586-0.785-1.347-0.785-2.178 0-0.076 0.002-0.151 0.007-0.225l-0.001 0.010v-10.344c0-0.414-0.336-0.75-0.75-0.75s-0.75 0.336-0.75 0.75v0 10.338c-0.018 0.146-0.029 0.315-0.029 0.486 0 2.221 1.741 4.035 3.932 4.152l0.010 0 0.034 0.002c0.001 0 0.003 0 0.005 0 1.073 0 2.040-0.452 2.721-1.176l0.002-0.002c0.766-0.858 1.235-1.996 1.235-3.244 0-0.053-0.001-0.107-0.003-0.16l0 0.008 0.057-14.283c0-7.775-11.844-7.775-11.844 0v15.518c-0.004 0.097-0.007 0.21-0.007 0.324 0 4.237 3.435 7.672 7.672 7.672 0.063 0 0.127-0.001 0.19-0.002l-0.009 0h0.017c0.024 0 0.053 0 0.082 0 2.038 0 3.898-0.767 5.305-2.029l-0.008 0.007c1.514-1.432 2.457-3.456 2.457-5.699 0-0.106-0.002-0.212-0.006-0.318l0 0.015 0.029-14.193c0-0.001 0-0.001 0-0.002 0-0.414-0.335-0.749-0.748-0.75h-0z"
            ></path>
          </svg>

          <span>Enviar Arquivos</span>
        </button>
      </li>
    </ol>
  </CMenu>

  <input
    type="file"
    ref="fileInput"
    class="hidden"
    @change="handleFileChange"
  />
</template>

<script>
import CMenu from './Menu.vue'
import FileCard from '@components/CFileCard.vue'
import { toast } from '@utils/toast'

export default {
  name: 'CChatInput',

  components: {
    CMenu,
    FileCard,
  },

  props: {
    placeholder: {
      type: String,
      default: 'Digite sua mensagem...',
    },
    disabled: {
      type: Boolean,
      default: false,
    },
    isLoading: {
      type: Boolean,
      default: false,
    },
    maxLength: {
      type: Number,
      default: null,
    },
    showCharCount: {
      type: Boolean,
      default: false,
    },
  },

  emits: ['send'],

  data() {
    return {
      message: '',
      isOpenMenuAction: false,
      files: [],
    }
  },

  computed: {
    canSend() {
      if (!this.message.trim()) return false
      if (this.maxLength && this.message.length > this.maxLength) return false
      return true
    },
  },

  watch: {
    isLoading(newVal) {
      if (!newVal && this.$refs.textareaRef) {
        this.$refs.textareaRef.focus()
      }
    },
  },

  methods: {
    handleInput() {
      if (this.$refs.textareaRef) {
        this.$refs.textareaRef.style.height = 'auto'
        const scrollHeight = this.$refs.textareaRef.scrollHeight
        const maxHeight = 128
        this.$refs.textareaRef.style.height = Math.min(scrollHeight, maxHeight) + 'px'
      }
    },

    handleKeyDown(event) {
      if (event.key === 'Enter' && !event.shiftKey) {
        event.preventDefault()
        if (this.canSend && !this.disabled) {
          this.handleSend()
        }
      }
    },

    handleSend() {
      if (!this.canSend || this.disabled) return
      const messageText = this.message.trim()
      if (!messageText) return
      this.$emit('send', messageText, this.files)
      this.message = ''
      this.files = []
      this.$nextTick(() => {
        if (this.$refs.textareaRef) {
          this.$refs.textareaRef.style.height = 'auto'
        }
      })
    },

    handleClickBtnMenu() {
      this.isOpenMenuAction = !this.isOpenMenuAction
    },

    openFilePicker() {
      this.$refs.fileInput.click()
    },

    handleFileChange() {
      const file = this.$refs.fileInput.files[0]
      if (file && this.checkIsvalidFileType(file)) {
        this.files.push(file)
        this.isOpenMenuAction = false
      } else {
        toast.error('Tipo de arquivo inválido', 5000)
      }
    },

    handleClickExcludeFile(file) {
      this.files = this.files.filter((f) => f !== file)
    },

    checkIsvalidFileType(file) {
      const validTypes = ['pdf', 'csv', 'txt', 'XLSX', 'md']
      const fileType = file.name.split('.')[1]?.toLowerCase()
      return validTypes.includes(fileType)
    },
  },
}
</script>

<style scoped>
.container-grid {
  display: grid;
  grid-template-columns: 3rem 1fr 3rem;
  grid-template-rows: repeat(2, auto);
  grid-gap: 1rem;

  width: 100%;

  grid-template-areas:
    "header header header"
    "after input action";
}

.header-template {
  grid-area: header;
}

.after-template {
  grid-area: after;
}

.action-template {
  grid-area: action;
}

.input-template {
  grid-area: input;
}

textarea::-webkit-scrollbar {
  width: 6px;
}

textarea::-webkit-scrollbar-track {
  background: transparent;
}

textarea::-webkit-scrollbar-thumb {
  background: #30363d;
  border-radius: 3px;
}

textarea::-webkit-scrollbar-thumb:hover {
  background: #484f58;
}
</style>
